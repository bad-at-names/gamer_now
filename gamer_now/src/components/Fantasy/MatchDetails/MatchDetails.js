		...
		if (this.props.matchId === 'Loading') {
			return (
				<div className="match-detail-container">
					...
					<h3 className="no-break">Select A Match on the Left to view Details</h3>
				</div>
			);
		} else {
				...
				mapsPlayed.map(maps => {
					...
					return (
						<div className="game-list" key={maps.id}>
							<h3 className="team-game-score">{maps.attributes.mapScore.team1}</h3>
								...
								<img
									src={mapIcon(
										maps.attributes.map
											...
											? 'rialto'
											: maps.attributes.mapGuid === '0x0800000000000891'
											? 'paris'
											: 'busan'
									)}
									className="map-pic"
									alt=""
								/>
								{console.log(maps.attributes.mapGuid)}
							</div>
							<h3 className="team-game-score">{maps.attributes.mapScore.team2}</h3>
						</div>
					);
				})
			) : (
			...
			return (
				<div className="match-detail-container">
					<h3 className="game-list">
						<h3 className="team-game-score">{this.props.matchId.competitors[0].name}</h3>
						<h3 className="map-pic-container">
							{this.props.matchId.scores[0].value} VS {this.props.matchId.scores[1].value}
						</h3>
						<h3 className="team-game-score">{this.props.matchId.competitors[1].name}</h3>
					</h3>
					{mapList}
				</div>
			);
